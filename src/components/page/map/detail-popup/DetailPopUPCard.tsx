...
export default function DetailPopUPCard({ data }: { data: PlaceDetailsType }) {
  ...
  return (
    <section className="rounded-t-lg absolute top-[13vh]  left-0 bottom-[10vh] z-10 w-screen p-2  bg-white shadow-lg sm:static sm:w-[30rem]">
      <div
        onClick={() => {
          router.replace("/map");
        }}
        className="w-full py-3 m-1 rounded-xl sm:hidden"
      >
        <div className="h-1 w-[80%] bg-gray-700 mx-auto rounded-full"></div>
      </div>
      <div className="h-full overflow-y-auto pb-[5vh]">
        <div className="relative flex items-center justify-center w-full my-1 rounded-lg shadow-md aspect-square">
          <XMarkIcon
            onClick={() => {
              router.replace("/map");
            }}
            className="absolute z-10 hidden h-8 duration-300 ease-out transform bg-white rounded-full shadow-lg cursor-pointer top-1 right-1 hover:scale-125 translate sm:inline-flex"
          />
          ...
          <>
            <section className="p-3 space-y-1 ">
              <p className="text-xs text-justify text-gray-700">
                The information above comes from googles database, no more
                further usefull information for this stablishment. If this place
                is yours, you can claim this place and provide more information{" "}
                <Link
